...
async def start(client, message):
	...
	I am file renamer bot, Please sent any telegram**Document Or Video** and enter new filename to rename it Created by:- @GUARDIANff"""
        await message.reply_photo(photo=LAZY_PIC,
                                caption=txt,
                                reply_markup=InlineKeyboardMarkup(
                                      [[InlineKeyboardButton("🔺 Update Channel 🔺", url="https://t.me/+Rm1Q1qVxhdg4ZWVl")],[InlineKeyboardButton("Movie Channel", url='https://t.me/+Rm1Q1qVxhdg4ZWVl')]
                                      ]))
        return
    if id:
        if old == True:
...
async def send_doc(client, message):
    ...
    try:
        ...
        await message.reply_text(text=f"Hello dear {message.from_user.first_name}  **we are currently working on this issue**\n\nPlease try to rename files from your another account.\nBecause this BOT can't rename file sent by some ids.\n\nIf you are an **ADMIN** Don't worry ! here we have a solution for you dear {message.from_user.first_name }.\n\nPlease use \n👉 `/addpremium your_other_userid` 👈 to use premium feautres\n\n",
                                  reply_markup=InlineKeyboardMarkup([
                                                                     [InlineKeyboardButton("🦋 Contact Developer 🦋", url='https://telegram.me/GUARDIANff')],[InlineKeyboardButton("  Support Group 🦋", url='https://t.me/+Rm1Q1qVxhdg4ZWVl')]
                                                                     ]))
        await message.reply_text(text=f"🦋")
        return 
